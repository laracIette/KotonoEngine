...
const std::vector<const char*> VulkanInstance::GetRequiredExtensions()
{
...
const bool VulkanInstance::IsDeviceSuitable(VkPhysicalDevice device)
{
...
const QueueFamilyIndices VulkanInstance::FindQueueFamilies(VkPhysicalDevice device) const
{
...
const SwapChainSupportDetails VulkanInstance::QuerySwapChainSupport(VkPhysicalDevice device) const
{
...
const VkResult VulkanInstance::CreateDebugUtilsMessengerEXT(VkInstance instance, const VkDebugUtilsMessengerCreateInfoEXT* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDebugUtilsMessengerEXT* pDebugMessenger) const
{
...
const VkSurfaceFormatKHR VulkanInstance::ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) const
{
...
const VkPresentModeKHR VulkanInstance::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) const
{
...
const VkExtent2D VulkanInstance::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) const
{
...
const VkShaderModule VulkanInstance::CreateShaderModule(const std::vector<char>& code) const
{
...
const VkCommandBuffer VulkanInstance::BeginSingleTimeCommands() const
{
...
void VulkanInstance::EndSingleTimeCommands(VkCommandBuffer commandBuffer) const
{
...
void VulkanInstance::RecordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) const
{
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
void VulkanInstance::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) const
{
...
const uint32_t VulkanInstance::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) const
{
...
void VulkanInstance::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
...
const VkFormat VulkanInstance::FindSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) const
{
...
const VkFormat VulkanInstance::FindDepthFormat() const
{
...
const bool VulkanInstance::HasStencilComponent(VkFormat format) const
{
...
const VkImageView VulkanInstance::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) const
{
...
void VulkanInstance::RecreateSwapChain()
{
    ...
    glfwGetFramebufferSize(Engine.GetWindow().GetGLFWWindow(), &width, &height);     // Replace to only freeze render
...
void VulkanInstance::CleanupSwapChain() const
{
...
const VkSampleCountFlagBits VulkanInstance::GetMaxUsableSampleCount() const
{
...
void VulkanInstance::OnFramebufferResized()
{
    _framebufferResized = true;
}

VkDevice VulkanInstance::GetDevice() const
{
    return _device;
}

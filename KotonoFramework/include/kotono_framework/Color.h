...
struct KtColor final
{
	...
	constexpr bool GetIsVisible() const noexcept { return a > 0.0f; }
	constexpr bool GetIsOpaque() const noexcept { return a >= 1.0f; }
	constexpr bool GetIsTransparent() const noexcept { return a < 1.0f; }
	...
	constexpr HSV GetHSV() const noexcept
	{
		...
		else if (max == rgb.r) hsv.h = 60.0f * ((rgb.g - rgb.b) / delta);
		else if (max == rgb.g) hsv.h = 60.0f * ((rgb.b - rgb.r) / delta + 2.0f);
		else hsv.h = 60.0f * ((rgb.r - rgb.g) / delta + 4.0f);
	...
	constexpr HSL GetHSL() const noexcept
	{
		...
		else
		{
			...
			if (max == rgb.r)
			{
				float temp = (rgb.g - rgb.b) / delta;
				if (temp < 0.0f) temp += 6.0f;
				hsl.h = 60.0f * temp;
			}
			else if (max == rgb.g) hsl.h = 60.0f * ((rgb.b - rgb.r) / delta + 2.0f);
			else hsl.h = 60.0f * ((rgb.r - rgb.g) / delta + 4);
	...
	static constexpr KtColor FromHex(const uint32_t hex) noexcept
	{
		return {
			((hex >> 24) & 0xFF) / 255.0f,
			((hex >> 16) & 0xFF) / 255.0f,
	...
	static constexpr KtColor FromHSV(const HSV& hsv, const float a = 1.0f) noexcept
	{
	...
	static constexpr KtColor FromHSL(const HSL& hsl, const float a = 1.0f) noexcept
	{
